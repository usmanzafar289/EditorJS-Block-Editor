...
class AttachesTool {
   ...
   */
  prepareUploadButton() {
    this.nodes.button = this.make('input', [this.CSS.apiButton, this.CSS.button]);
    this.nodes.button.type = "file";
    this.nodes.button.innerHTML = ` ${this.config.buttonText}`;
    this.nodes.button.addEventListener('change', this.enableFileUpload);
  ...
  enableFileUpload() {
    ...
    //   let contents = fr.readAsText(file);
    //   document.getElementById(elementId).innerText = contents;
    const attributes = {
      src: file,
    };
    this.nodes.imageEl = this.make("img", null, attributes);
    this.data = {
      file: {
        file,
